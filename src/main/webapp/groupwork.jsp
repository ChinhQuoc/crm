									...
									<tbody>
										<c:choose>
											<c:when test="${ projects == null }">
												<tr>
													<td colspan="5">Không có project nào</td>
												</tr>
											</c:when>
											<c:otherwise>
												<c:forEach var="item" items="${ projects }" varStatus="position">
													<tr>
														...
														<td>${item.endDate}</td>
														<td>
															<a href=<c:url value="/groupwork-edit?id=${ item.id }" /> class="btn btn-sm btn-primary">Sửa</a>
															<a id-project="${item.id}" href="#" class="btn btn-sm btn-danger btn-xoa">Xóa</a>
															<a href=<c:url value="/groupwork-detail?id=${ item.id }" /> class="btn btn-sm btn-info">Xem</a>
														</td>
													</tr>
												</c:forEach>
											</c:otherwise>
